...
def require_api_key(request: Request):
    ...
    In real systems you'd likely use OAuth/JWT, mTLS, or a proper gateway.
    """
    if not EDGE_API_KEY:
...
def metrics():
    """Very small "metrics" output to teach the idea of counters.
    (Not Prometheus format-complete on purpose — this is a lab.)
    """
    return (
...
async def chat(payload: ChatIn, request: Request):
    """RAG endpoint: retrieve top-K sources from Weaviate, then generate an answer with Ollama."""
...
async def debug_retrieve(request: Request, q: str = Query(min_length=2, max_length=2000)):
    ...
    This is useful to prove whether your issue is retrieval or generation.
    """
    require_api_key(request)
    try:
...
async def debug_prompt(payload: ChatIn, request: Request):
    ...
    This helps students understand how grounding works.
    """
    require_api_key(request)
    try:
        sources = await retrieve_sources(payload.message)
